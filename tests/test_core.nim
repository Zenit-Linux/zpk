...
""")
    ...
    # osobny plik `<plik>.zpk.json`. v0.5: archiwum jest w natywnym
    # formacie ZPKA (nie tar) -- sprawdzamy to przez `archive.listMembers`.
    check not fileExists(path & ".json")
    check archive.isZpkaFile(path)
    let (listOk, members, listErr) = archive.listMembers(path)
    check listOk
  ...
  test "verifyPackage wykrywa niezgodność sha256 (manifest w środku archiwum)":
    ...
    # v0.5: budujemy archiwum RĘCZNIE przez `archive.writeArchive` (format
    # ZPKA), zamiast przez `tar` jako proces potomny -- ten sam efekt
    # (pełna kontrola nad jawnie błędną sumą w manifeście), zero zależności
    # zewnętrznych nawet w samym teście.
    ...
    discard archive.writeArchive(zpkPath, toPack)
...
suite "zsha256 (sha256 w czystym Nim, bez procesów potomnych)":
  ...
  test "sha256HexOfFile zgodne z sha256Hex dla tej samej zawartości":
    let dir = createTempDir("zpktest-sha", "")
    defer: removeDir(dir)
    let content = "przykladowa zawartosc pliku do testu\n".repeat(500)
...
suite "zlz (kompresor LZ77 w czystym Nim, format ZLZ1)":
  test "round-trip: pusty, krótki, powtarzalny, losowy":
    for sample in ["", "a", "ab", "abcd", "abcdefgh".repeat(2000)]:
      check zlz.decompress(zlz.compress(sample), sample.len) == sample

  test "kompresja realnie zmniejsza rozmiar dla danych powtarzalnych":
    let data = "x".repeat(10_000)
    let compressed = zlz.compress(data)
    check compressed.len < data.len div 10
...
suite "archive (natywny format ZPKA -- zastępuje tar)":
  test "writeArchive/readIndex/extractAll: pełna zgodność bajt-w-bajt":
  ...
  test "extractMember czyta tylko jeden człon (manifest.json)":
  ...
  test "isZpkaFile odrzuca pliki spoza formatu (np. stary tar/inny plik)":
